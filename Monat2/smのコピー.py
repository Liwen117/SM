...
#!attibute?
#H=np.ones((RA,SA))+1j*np.ones((RA,SA))
H=np.random.rand(RA,SA)+1j*np.random.rand(RA,SA)
##Channel matrix
SNR_noise_dB=20
SNR_RA_dB=15
...
#r_BBr_MF=r.Matched_Filter(np.real(r_BB),g,sps)  
#r_BBi_MF=r.Matched_Filter(np.imag(r_BB),g,sps) 
