...
receiver_=receiver(H,H,sender_,s,SNR_noise_dB,SNR_RA_dB,filter_,mpsk_map)
r=receiver_.channel()
r_mf=receiver_.Matched_Filter(r.real)+1j*receiver_.Matched_Filter(r.imag)
BERi,BERd=test.BER(receiver_.yi,receiver_.yd,Ni,Nd,ibits,dbits)
...
H_est=H_est/np.repeat(i,RA).reshape(-1,RA).transpose()
#f sychr
off=np.exp(1j*2*np.pi*(-f_est)*np.arange(r_n_syc.shape[0])*T)
r_ft_syc=r_n_syc*np.repeat(off,RA).reshape([-1,RA])
#BERi,BERd=test.BER(receiver_.yi,r_ft_syc,Ni,Nd,ibits,dbits)
