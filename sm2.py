...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 11 11:36:05 2017
...
#        rx[i,j]=np.sqrt(10**(SNR_dB / 10))*symbols[i]*H[j,s_a_index[i]]
#        rx[:,j]=rx[:,j]+n
receiver_=receiver(H,sender_,SNR_dB,filter_,mpsk_map)
rx=receiver_.channel_nf(n_up)
yi,yd=receiver_.detector(rx,H)      
BERi_ohneoffsets,BERd_ohneoffsets=test.BER(yi,yd,Ni,Nd,ibits,dbits)        
...
#symbols=sender_.symbols
#ibits=sender_.ibits
#receiver_=receiver(H,H_est,sender_,s,SNR_dB,filter_,mpsk_map)
#r=receiver_.channel()
#r_mf=receiver_.Matched_Filter(r.real)+1j*receiver_.Matched_Filter(r.imag)
